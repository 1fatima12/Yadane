...
class BtiResourceIT {
    ...
    public static Bti createEntity(EntityManager em) {
        Bti bti = new Bti().numOrdre(DEFAULT_NUM_ORDRE).date(DEFAULT_DATE).qte(DEFAULT_QTE);
        return bti;
    }
    ...
    public static Bti createUpdatedEntity(EntityManager em) {
        Bti bti = new Bti().numOrdre(UPDATED_NUM_ORDRE).date(UPDATED_DATE).qte(UPDATED_QTE);
        return bti;
    }
    ...
    void createBti() throws Exception {
        ...
        // Create the Bti
        restBtiMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(bti)))
    ...
    void putNewBti() throws Exception {
        ...
        updatedBti.numOrdre(UPDATED_NUM_ORDRE).date(UPDATED_DATE).qte(UPDATED_QTE);

        restBtiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedBti.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(updatedBti))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingBti() throws Exception {
        ...
        restBtiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, bti.getId()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(bti))
            )
            .andExpect(status().isBadRequest());
        ...
        restBtiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, count.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(bti))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamBti() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restBtiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(bti)))
    ...
    void fullUpdateBtiWithPatch() throws Exception {
        ...
        partialUpdatedBti.numOrdre(UPDATED_NUM_ORDRE).date(UPDATED_DATE).qte(UPDATED_QTE);
    ...
    void patchNonExistingBti() throws Exception {
        ...
        restBtiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, bti.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(bti))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamBti() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restBtiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(bti)))
            .andExpect(status().isMethodNotAllowed());
